...
def predict(sample):
    ...
    xlnet_result = np.where(XLNet_predictions == np.max(XLNet_predictions))[0][0]
    bert_result = np.where(bertPredictions == np.max(bertPredictions))[0][0]
    roberta_result = np.where(Roberta_predictions == np.max(Roberta_predictions))[0][0]
    result.append(xlnet_result)
    result.append(bert_result)
    result.append(roberta_result)
    print(result)
    winner = most_common(result)
    print(winner)
